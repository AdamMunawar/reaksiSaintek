  ...
  try {
    ...
    if (process.env.DATABASE_URL) {
      ...
      const res = await query(
        `INSERT INTO rubriks (id, slug, name, description, color)
         VALUES ($1, $2, $3, $4, $5)
         ON CONFLICT (slug) DO UPDATE
         SET name = EXCLUDED.name, description = EXCLUDED.description, color = EXCLUDED.color
         RETURNING *;`,
        [rubrikId, slug.toLowerCase().trim(), name.trim(), description || '', color || '#2563EB']
      );
    ...
    const saved = db.saveRubrik({ id, name, slug, description, color, emoji: '' });
